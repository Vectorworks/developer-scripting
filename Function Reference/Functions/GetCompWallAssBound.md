...
## Examples
```pascal
resultOK := GetCompWallAssBound(object, 1, 2);
```
```python
import vs
...
ok, wallAssociatedBound = vs.GetCompWallAssBound(object, componentIndex)
vs.Message('GetCompWallAssBound returned: ' + str((ok, wallAssociatedBound)))
```
