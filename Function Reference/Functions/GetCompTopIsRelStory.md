...
## Examples
```pascal
resultOK := GetCompTopIsRelStory(object, 1, TRUE);
```
```python
import vs
...
ok, topIsRelativeToStory = vs.GetCompTopIsRelStory(object, componentIndex)
vs.Message('GetCompTopIsRelStory returned: ' + str((ok, topIsRelativeToStory)))
```
