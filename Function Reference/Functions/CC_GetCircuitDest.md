...
## Examples
```pascal
CC_GetCircuitDest(hCircuit, hDevice, hDevSkt, hAdapter, hSocket);
```
```python
import vs
...
hDevice, hDevSkt, hAdapter, hSocket = vs.CC_GetCircuitDest(hCircuit)
vs.Message('CC_GetCircuitDest returned: ' + str((hDevice, hDevSkt, hAdapter, hSocket)))
```
