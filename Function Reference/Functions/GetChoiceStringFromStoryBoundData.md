...
## Examples
```pascal
GetChoiceStringFromStoryBoundData(1, 2, 'Design Layer-1', 'Example');
```
```python
import vs
...
text = vs.GetChoiceStringFromStoryBoundData(boundType, boundStory, layerLevelType)
vs.Message('GetChoiceStringFromStoryBoundData returned: ' + str(text))
```
